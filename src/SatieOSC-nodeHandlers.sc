+ SatieOSC {
	...
	// expects /oscaddress groupName nodeType
	createGroupHandler {
		^{ | args |
			...
			if ( (args.size < 2 ) ,
				{
					"→    %: message: bad arg count: expects at least 1 value:  nodeName,  and optionally a second value for nodeType, e.g.: 'effect' ".format(this.class.getBackTrace).error
				},
				// else
				{
					...
					if (args.size == 3,
						{
							if (args[2].asSymbol == \effect, {position = \effect;
							});
